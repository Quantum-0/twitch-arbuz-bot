...
async def cmd_bite_handler(chatbot: "ChatBot", channel: str, message: ChatMessage):
    ...
    bites_count = await sm.get_state(channel=channel, user=user, command="bite", param=SMParam.COUNT)
    if last_bite and time() - last_bite < 15:
        if bites_count > 3:
            ...
            return
        await sm.set_state(channel=channel, user=user, command="bite", param=SMParam.COUNT, value=bites_count+1)
    else:
        await sm.set_state(channel=channel, user=user, command="bite", param=SMParam.COUNT, value=1)
