...
class PatCommand(SimpleTargetCommand):
    ...
    async def _no_target_reply(self, user: str) -> str | None:
        return random.choice(
            [
                'Чтобы кого-нибудь пат-патнуть, нужно указать цель! Например "!pat @Quantum075Bot"',
    ...
    async def _cooldown_reply(self, user: str, delay: int) -> str | None:
        return random.choice(
            [
                *[
                    f"@{user}, подожди, пожалуйста, {delay_to_seconds(delay)}, а то сейчас кому-нибудь лысину сделаешь своими поглаживаниями о:"
                ]
                * 3,
    ...
    async def _bot_call_reply(self, user: str, target: str) -> str | None:
        return random.choice(
            [
                "Правильно, боты тоже заслуживают поглаживаний ^w^",
