...
async def load_supporters(cli: MemealertsAsyncClient, *, db_session_factory=None) -> list[Supporter]:
    limit = 100
    ...
    if total_count <= limit:
        if db_session_factory:
            try:
                await save_all_supporters_into_db(items, db_session_factory=db_session_factory)
            except:
...
async def find_user_in_supporters(
    cli: MemealertsAsyncClient, username: str, *, db_session_factory=None,
) -> Supporter | None:
    users = await load_supporters(cli, db_session_factory=db_session_factory)
...
async def find_and_give_bonus(
    cli: MemealertsAsyncClient, username: str, amount: int = 2, *, db_session_factory = None,
) -> bool:
    if is_id(username):
    ...
    user_in_supporters = await find_user_in_supporters(cli, username, db_session_factory=db_session_factory)
...
async def give_bonus(memealerts_token, streamer, supporter, amount, *, db_session_factory=None):
    ...
    async with MemealertsAsyncClient(memealerts_token) as meme_cli:
        result = await find_and_give_bonus(meme_cli, supporter, amount, db_session_factory=db_session_factory)
        return result
